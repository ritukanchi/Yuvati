<script>
    ...
    async function fetchDiseaseArticle() {
        try {
            loading = true;
            const articleRef = doc(db, "products", "tampon");
            ...
            if (articleSnap.exists()) {
                ...
                if (data?.tampon) {  // Adjusting to the 'tampon' field
                    article = {
                        id: articleSnap.id,
                        title: "",
                        content: data.tampon,  // Fetching the 'tampon' field
                    };
                } else {
                    throw new Error("Tampon field not found in the document");
                }
            } else {
            ...
            error = err.message;
            console.error("Error fetching tampon article:", err);
        } finally {
            loading = false;
        }
    }
